...
void hop(Carrier * c, Site * dest, Vector * dist,
...
hoppingStep(Site * sites, Carrier * carriers, Results * res, bool stat)
{
    ...
    if(dest->s->carrier == NULL)
    {
        ...
        hop(c, dest->s, &dest->dist, res, carriers, stat);
    }
    else
    {
        if(stat)
 ...
 */
void
hop(Carrier * c, Site * dest, Vector * dist,
	Results * res, Carrier * carriers, bool stat)
{
    Site * orig = c->site;
    ...
    if(stat)
    {
        ...
        dest->tempOccTime   = res->simulationTime;
    ...
    dest->carrier = c;
    if(stat && orig->energy < dest->energy)
        dest->visitedUpward++;
    else if(stat)
        dest->visited++;
